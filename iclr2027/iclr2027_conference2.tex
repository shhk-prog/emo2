...
H1のprespecified replication criterionは、
\[
CI_{\mathrm{low}}
\left(
...
ただし、このQwen上のpatternをfamily-generalなspatiotemporal lawとして一般化することはできない。Gate failure後に探索的に解釈したprespecified cross-family replication analysisでは、Valence軸においてLlama 3.2およびOLMo 2がQwen Discoveryと同方向の空間的解離（因果ピークおよび重心がデコーディングより浅い層に位置する）を有意に再現した（表\ref{tab:v3_confirmatory_details}）。しかし、Gemma 3ではValenceの解離が支持されず、またArousal軸ではいずれのfamilyにおいても信頼区間が0を跨ぐか重心方向が一致しなかったため、prespecified directional replication criterionは3 families全体・Valence/Arousal双方に一貫して一般化するpatternとはならなかった。
...
ただしState Induction GateがNO\_GOであり、H1--H3のprespecified replication criteriaは満たされなかったため、このtemporal contrast単独から普遍的なcausal mechanismを同定したとは言えない。
