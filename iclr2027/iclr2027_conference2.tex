...
\begin{abstract}
大規模言語モデル（LLM）の内部状態から情動関連情報を線形デコードできることは、その情報が表現内にアクセス可能な形で存在することを示す。しかし、表層的な行動の共変（Behavioral Coupling）やデコード可能性（Decodability）が、モデル自身による自己報告生成において当該表現を直接・普遍的に因果的利用していること（Causal Leverage）を意味するわけではない。本研究では、人間アノテーションに基づく情動刺激（EmoBankおよび統制されたAIPsy-Affect）を用い、他者予測（Reader Prediction）と自己報告（Self-Report）の関係を4つのモデルファミリー（Qwen 2.5, Llama 3.2, Gemma 3, OLMo 2）にわたって4段階のエビデンス階層（Behavioral $\to$ Internal Representation $\to$ Post-training-Associated Reorganization $\to$ Spatiotemporal Causal Utilization）で体系的に検証した。実験の結果、(1) 出力レベルでの強固な変位共変（Behavioral Coupling）にもかかわらず、内部表現の直接共有や因果的交換可能性は限定的であること、(2) 事後学習（Post-training）に伴う変化は情報の完全な消去（Erasure）ではなく表現幾何（Geometry）とタスク依存的因果プロファイルの再編成（Reorganization）として現れること、(3) 状態誘導ゲート（State Induction Gate）はNO\_GOとなり、局所的デコード可能性が直接的かつ頑健な因果制御可能性を保証しないこと、(4) 追試検証において生成ステージ依存的な因果感度が確認されたものの、普遍的な単一因果メカニズムには至らないことを実証した。これらの知見は、心理的支援における他者理解能力を保持しつつ過度な自己感情表出を選択的に制御するガードレール設計に向け、表層出力・表現・因果利用の多重の解離（Separation）を明示的に扱う重要性を示す。
\end{abstract}
...
この知見は、LLMに主観的感情が存在することを示すものではない。むしろ、心理的支援に必要な他者理解能力を維持しながら、
過度なself-directed affective responseを選択的に制御するためには、behavioral outputだけでなく、
\[
\text{representation}
\neq
\text{causal utilization}
