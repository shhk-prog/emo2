...
\begin{abstract}
大規模言語モデル（LLM）は、文章中の感情を高精度に推定できる一方、同じ刺激に対して自身の状態を報告させた場合、その応答が内部の情動関連表現をどの程度反映しているかは明らかでない。本研究では、LLMの情動関連内部表現と自己報告の関係を、行動、内部表現、事後学習、計算過程の4つのレベルから統合的に検証する。
まず、統制された情動刺激に対する他者感情推定（Reader Prediction）と自己報告（Self-Report）の共変動を測定し、両者の行動的対応を特徴づける。次に、線形プロービング、表現幾何解析、および因果介入を用いて、ReaderとSelfの背後にある情動関連表現と因果的感受性がどの程度共有されるかを検証する。さらに、BaseモデルとInstruction-tunedモデルの対応比較を通じて、事後学習に伴う表現幾何およびrepresentation–report couplingの再編を解析する。最後に、方向介入、部分空間除去、および生成過程に沿った時空間的介入を用い、内部にデコード可能な情動情報が、いつ、どの層・計算段階で自己報告に対する因果的影響を獲得するかを特定する。
一連の分析は、自己報告が単なる出力レベルの共変動でも、内部に存在するすべてのデコード可能な情動情報の直接的な読み出しでもないことを示す。むしろ、情動関連情報と自己報告の関係は部分的かつタスク依存であり、事後学習に伴って再編され、計算過程の特定の段階においてのみ自己報告に対する因果的レバレッジを持つ。本研究は、LLMにおける「情報が表現されていること」と「その情報が実際の出力生成に利用されること」を区別し、内部表現から自己報告へ至る計算的経路を統一的に明らかにする。
\end{abstract}
