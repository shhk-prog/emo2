...
\begin{abstract}
大規模言語モデルを心理的支援や対話に利用する上では、他者の情動を理解する能力を維持しつつ、過度な情動表現を選択的に制御する心理的ガードレールが重要となる。しかし、内部状態から情動関連情報をデコードできることは、その情報がSelf-Report生成に因果的に利用されていることを意味しない。本研究では、EmoBankとAIPsy-Affectを用い、Reader PredictionとSelf-Reportの関係を4モデルファミリーについて、behavior、internal representation、post-training-associated reorganization、spatiotemporal causal utilizationの4段階から検証した。ReaderとSelfの出力変位は強く共変し、双方からaffect-relevant informationをデコードできた一方、表現の直接共有やcross-task causal interchangeabilityは限定的であった。Base--Instruct間では情報は保持されつつrepresentation geometryとcausal profileが再編され、単純なinformation erasureや一様なdepth relocationでは説明できなかった。さらにState Induction GateはValence/Arousal双方で\texttt{NO\_GO}となり、decodabilityがrobustなSelf-Report causal controlを保証しないことが示された。以上より、\(\text{Behavioral Coupling}\neq\text{Shared Representation}\neq\text{Shared Causal Code}\neq\text{Robust Causal Control}\)という段階的分離を示し、心理的ガードレールには表層behaviorではなくtask-specificなcausal utilizationを明示的に評価する必要があることを示す。
\end{abstract}
...
単一のgreedy responseではなく、事前定義した全JSON candidateのconditional sequence likelihoodを計算し、そのSoftmax分布からexpected Valence/Arousal等を求める。Behavioral/V1では729-state VAD space、V2/V3では81-state VA spaceを用いる。両者は異なるprobability spaceであるため、Stage間でabsolute expected scoreを直接比較せず、各Stage内部で定義したcontrast、geometry、causal effect、relative depthを比較する。
...
本研究の再現性を確保するため、使用したデータセット、対象モデルとmodel revision、プロンプト、candidate space、Sequence-Likelihoodによる出力測定、hidden-stateの測定位置、random seed、統計解析、および各StageにおけるPrimary/Secondary analysisの定義をAppendixに記載する。
\color{red}
コードに関する記述を削除
% また、Behavioral、V1、V2、V3の主要実験、control analysis、および論文中の集計値を生成するための実装と設定を付属コードとして提供する。
\color{black}
Base--Instruct比較ではprompt-format differenceを統制するMatched-Plain conditionをPrimaryとし、V3ではDiscoveryとcross-family replicationを分離するとともに、State Induction Gateが\texttt{NO\_GO}となった後の解析をexploratory/descriptiveとして明示的に区別している。これらにより、主要な実験条件、解析手順、および論文中の報告値を追跡・再現できるようにする。
...
\]
となり、Primary interactionの中で唯一FDR補正後にも有意に支持された（さらにSecondaryのPost-training主効果も $q = 0.027$ で有意）。
