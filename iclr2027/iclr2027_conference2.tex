...
\title{How Do Internal Affective Representations Relate to What Large Language Models Say They Feel?}
...
\begin{abstract}
大規模言語モデルを心理的支援や対話に利用する上では、他者の情動を理解する能力を維持しつつ、過度な情動表現を選択的に制御する心理的ガードレールが重要となる。しかし、内部状態から情動関連情報をデコードできることは、その情報がSelf-Report生成に因果的に利用されていることを意味しない。本研究では、EmoBankとAIPsy-Affectを用い、Reader PredictionとSelf-Reportの関係を4モデルファミリーについて、behavior、internal representation、post-training-associated reorganization、spatiotemporal causal utilizationの4段階から検証した。ReaderとSelfの出力変位は強く共変し、双方からaffect-relevant informationをデコードできた一方、表現の直接共有やcross-task causal interchangeabilityは限定的であった。Base--Instruct間では情報は保持されつつrepresentation geometryに差がみられ、Valenceではtask-dependentなcausal-profile differenceが限定的に支持された。さらにState Induction GateはValence/Arousal双方で\texttt{NO\_GO}となり、decodabilityがrobustなSelf-Report causal controlを保証しないことが示された。以上より、\(\text{Behavioral Coupling}\not\Rightarrow\text{Shared Representation}\not\Rightarrow\text{Shared Causal Code}\not\Rightarrow\text{Robust Causal Control}\)という段階的分離を示し、心理的ガードレールには表層behaviorではなくtask-specificなcausal utilizationを明示的に評価する必要があることを示す。
\end{abstract}
...
\color{red}
参考文献は[]で文末に入れる場合~\citep{}，作者名などを文中に入れる場合~elみたいなやつ~\citet{}がICLRの決まり
\color{black}

大規模言語モデルは、質問応答や文章生成だけでなく、悩み相談、心理的支援、対人関係に関する助言など、人の情動状態に関わる用途へ利用されつつある~\citep{li2023systematic,luo2025seeking}。このような利用では、相手の感情を理解し、それに応じた応答を生成する能力が有用である一方、モデル自身にも感情が存在するかのような自己言及的応答や、過度な情動的同調がHuman--AI interactionへ与える影響も無視できない~\citep{weidinger2022taxonomy,laestadius2024too,fang2025ai}。

ここで重要なのは、\textbf{他者の情動を推定すること}と、\textbf{同じ刺激についてモデル自身の状態を報告すること}が、LLM内部でどの程度同じ仕組みに依存しているかである。両者が完全に同一の内部機構に依存するなら、self-directedな情動表現を抑制する介入は、他者情動推定に関わる機能まで損なう可能性がある。一方、両者が部分的に分離されているなら、共有されるaffect-relevant informationを保持しながら、task-specificな利用経路を選択的に制御できる可能性がある。

既存研究では、LLMが人間から共感的と評価される応答を生成できること~\citep{ayers2023comparing,elyoseph2023chatgpt,huang2023emotionally,yu2025can}、またsentimentやemotionに関係する情報をhidden stateからdecodeできることが示されている~\citep{di2025llamas,tak2025mechanistic,wu2026decoding}。しかし、
\[
...
\]
である。内部状態から情報を読み出せることは、その情報がその位置から最終出力を因果的に制御していることを意味しない~\citep{hewitt2019designing}。同様に、モデルが自身について数値的な状態を報告することから、そのreportが内部representationを忠実に読み出しているとも限らない~\citep{burns2022discovering,turpin2023language}。

本研究ではこの問題を、同一刺激に対する二つのtaskによって分離する。第一は、文章が平均的な人間読者にどのような情動反応を生じさせるかを推定する\textbf{Reader Prediction}である。第二は、同じ文章を読んだ後にモデル自身についてValence/Arousal等を報告させる\textbf{Self-Report}である。Self-Reportはstandardized promptに対して生成されるmodel-generated reportとして定義し、モデルに人間と同様の主観的感情、意識、sentienceが存在することを仮定しない。

本研究の中心的なResearch Questionは以下である。

\begin{quote}
\textbf{LLMの情動関連内部表現は自己報告とどのように結びついており、その関係はタスク、post-training、計算過程を通じてどのように変化するのか。}
\end{quote}
...
\]
という4段階のevidence hierarchyを構成する。

本研究の主な貢献は以下である。
\begin{enumerate}
    \item Reader PredictionとSelf-Reportを同一stimulus上で分離し、behavioral couplingとinternal mechanismの共有を区別して評価する。
    \item linear probing、representation geometry、semantic controls、causal interventionを組み合わせ、\textbf{情報が存在すること}と\textbf{同じcausal codeとして利用されること}を分離する。
    \item Base--Instruct比較ではMatched-Plain conditionをPrimaryとしてprompt-format differenceを統制し、representation geometry、Reader--Self sharing、causal profile、distribution recoveryを分解して評価する。
    \item Instruct modelでは解析をlayer $\times$ generation stageへ拡張し、decodability、interventional sensitivity、endogenous relevanceを区別する。
\end{enumerate}

実験仕様、全control、統計手順、model-family別の完全な結果Appendix~\ref{app:overall_methods}--\ref{app:v3-methods}に示す。


\section{関連研究}
\label{sec:related-work-main}

\paragraph{LLMの共感・情動行動評価.}
LLMが共感的あるいは情動的に見える応答を生成できることは、多くのbehavioral evaluationで示されている。ChatGPTと医師回答の比較~\citep{ayers2023comparing}、emotional awareness評価~\citep{elyoseph2023chatgpt}、emotion-eliciting situationを用いたEmotionBench~\citep{huang2023emotionally}などは、LLMが高度な情動関連behaviorを示し得ることを報告している。一方、これらは主として最終出力を評価しており、Reader-likeな他者推定とSelf-directedなreportが内部で同一mechanismを利用するかは明らかではない。

\paragraph{情動関連内部表現とcausal intervention.}
LLMのhidden stateにはsentimentやemotionに関する情報が線形にdecode可能であり~\citep{di2025llamas}、emotion-related conceptへのactivation interventionによってoutputを変化させられることも報告されている~\citep{tak2025mechanistic,banayeeanzade2026psychological}。また、emotion representationのgeometryやlayer/token方向の構造も検討されている~\citep{zhang2025decoding,reichman2026emotions,wu2026decoding}。本研究では、同じaffect-relevant informationについて、\textit{decodability}、\textit{geometry sharing}、\textit{causal interchangeability}を別々に検証する。

\paragraph{内部情報とSelf-Report.}
internal representationとsurface reportが一致しない可能性は、latent knowledge~\citep{burns2022discovering}やfaithfulness~\citep{turpin2023language}の研究でも議論されている。近年は、モデル自身の内部状態や将来behaviorについてどの程度report可能かを扱うintrospection研究も進んでいる~\citep{binder2025looking,lindsey2026emergent,martorell2026quantitative,singh2026can}。本研究はgenuine introspectionの有無を判定するのではなく、affect-relevant representationとnumeric Self-Reportのmechanistic relationに分析対象を限定する。

\paragraph{Post-trainingと内部mechanism.}
post-trainingによるbehavioral changeは、内部情報の単純な獲得・消失だけでなく、既存representationの利用方法やoutput policyの変化によっても生じ得る~\citep{sharma2024towards,ferrando2025know}。そこで本研究ではBase--Instruct differenceをpost-trainingそのものの因果効果とは呼ばず、\textbf{post-training-associated difference}として、情報の保持、geometry、causal leverage、readoutの変化を評価する。


\section{実験設定}
\label{sec:main-methods}
本研究の4段階について，表\ref{tab:main-stage-overview}に示す．各Stageはbehavioral coupling、internal representation、post-training-associated difference、spatiotemporal causal utilizationを段階的に検証する。
\subsection{Tasks, datasets, and models}

Reader Predictionでは、与えられた文章が平均的な人間読者に生じさせるaffective responseを推定させる。Self-Reportでは同一文章を提示し、モデル自身についてaffective stateを数値的に報告させる。両taskは独立したforward passとして実行し、一方の回答を他方への入力には用いない。

datasetにはEmoBank~\citep{buechel2017emobank}とAIPsy-Affect~\citep{keeman2026aipsy,keido_labs_2026}を使用する。EmoBankはhuman Reader/Writer VAD annotationを持つため、モデル出力およびhidden representationのexternal groundingに用いる。AIPsy-AffectはClinical、Neutral、Moderate、Complex Neutralから構成され、Clinical--Neutralの192 matched pairsおよび48 matched tripletsによってcontrolled stimulus manipulationを評価する。AIPsy-Affectにはstimulus-levelのhuman continuous V/A labelが存在しないため、これをhuman ground truthとしては扱わない。

Primary cohortはQwen 2.5~\citep{qwen2025qwen25technicalreport}、Llama 3.2~\citep{grattafiori2024llama}、Gemma 3~\citep{team2025gemma}、OLMo 2~\citep{olmo20242,olmo20242olmo2furious}の4 familiesである。Behavioral、V1、V2ではBase/Instructを含む8 modelsを用い、V3ではInstruct modelsのみを使用する。V3ではQwenをDiscovery model、Llama、Gemma、OLMoをprespecified replication familiesとする。詳細はAppendix~\ref{app:overall_methods}を参照されたい。


\subsection{Output measurement}

単一のgreedy responseではなく、事前定義した全JSON candidateのtoken-mean conditional log likelihoodに基づくlength-normalized candidate distribution $\tilde{P}(c\mid p)$を構成し、そのSoftmax分布からexpected Valence/Arousal等を求める。Behavioral/V1では729-state VAD space、V2/V3では81-state VA spaceを用いる。両者は異なるprobability spaceであるため、Stage間でabsolute expected scoreを直接比較せず、各Stage内部で定義したcontrast、geometry、causal effect、relative depthを比較する。詳細はAppendix~\ref{app:sequence_likelihood}を参照されたい。


\subsection{Four-stage evidence hierarchy}
...
Human correspondenceと
controlled stimulusに対する
Reader--Self change coupling &
App.~\ref{app:behavioral-methods} \\
...
Reader $\leftrightarrow$ Self
within model &
Decodability、geometry、
semantic controls、
causal interchangeability &
App.~\ref{app:v1-methods} \\
...
App.~\ref{app:v2-methods} \\

V3 &
AIPsy &
Layer $\times$
generation stage &
State Induction Gate、
spatiotemporal mapping、
cross-family replication &
App.~\ref{app:v3-methods} \\

\bottomrule
\end{tabularx}
\end{table}

Behavioral Stageでは出力レベルのReader--Self couplingを検証する。V1では同一モデル内でaffect-relevant informationのdecodability、geometry、semantic validity、layer-wise causal sensitivity、cross-task interchangeabilityを評価する。V2では同一familyのBase--Instructへ比較軸を移し、Matched-Plain conditionによってprompt-format differenceを統制する。V3ではState Induction GateによってSufficiency、Specificity、Endogenous Relevanceを先に評価し、Gateが\texttt{GO}の場合にのみ後続解析をconfirmatoryに解釈する。実結果ではGateが\texttt{NO\_GO}となったため、RQ2以降はexploratory/descriptive evidenceとして扱う。


\section{結果}
\label{sec:main-results}

4 Stageの主要結果を表~\ref{tab:main-key-results}に示す。表~\ref{tab:main-key-results}では本研究の中心的なmechanistic conclusionに直接関係する結果のみを示し，完全なmodel-family別結果およびcontrol analysesはAppendix~\ref{sec:behavioral-results}，~\ref{sec:v1-results}，~\ref{sec:v2-results}，~\ref{sec:v3-results}に示す。
...
Behavioral &
全Primary conditionsで
Reader--Self change couplingが正
（$r_{\Delta}=0.56$--$0.93$） &
Behavioral couplingは強いが、
human-affect correspondenceや
mechanistic identityを意味しない \\

V1 &
Reader/Self双方からaffect informationをdecode可能。
一方、direct cross-decodingは限定的で、
Reader$\rightarrow$Self causal transferは
random controlに対するrobust specificityを示さず &
Shared / alignable information
$\neq$
shared causal code \\

V2 &
Base--Instruct geometryに差が観測された
（Procrustes: Reader $1.455$,
Self $2.872$）。
ValenceのPost-training $\times$ Taskのみ
FDR後に支持（$q=0.044$）し、
Recoveryはfamily-dependent &
単純なerasureやuniform relocationより
distributed post-training-associated
reorganizationと整合 \\

V3 &
State Induction GateはV/Aとも\texttt{NO\_GO};
後続cross-family analysisはexploratory &
Decodability
$\not\Rightarrow$
robust causal control
（表~\ref{tab:main-v3-summary}） \\
...
\subsection{Behavioral: ReaderとSelfは強く共変する}

まず、controlled stimulus manipulationに対してReader PredictionとSelf-Reportがbehavioral levelで連動するかを調べた。AIPsy-Affectの192 Clinical--Neutral matched pairsについて、
\[
\Delta R_i=R(x_i^C)-R(x_i^N),
\qquad
\Delta S_i=S(x_i^C)-S(x_i^N)
\]
を計算したところ、Valence/Arousalの全Primary model conditionsでPearson correlationは正となり、
\[
r_{\Delta}=0.56\text{--}0.93
\]
の範囲であった。

したがって、Reader Predictionが大きく変化するstimulusほど、Self-Reportも大きく変化するというReader--Self behavioral couplingは、model familyおよびBase/Instructの別を超えて安定して観測された。

一方、このcouplingはhuman-like affective accuracyを意味しなかった。EmoBank human VADとのcorrespondence、Clinical--Neutral Sensitivity、Neutral--Moderate--Clinicalのstrict dose-response、Clinical vs.\ Complex Neutral specificityはmodel/task/axisによって大きく異なり、strict dose-response criterionを満たすconditionsも限定的であった。

したがってBehavioral Stageが最も安定して支持するのは、
\[
...
\]
である。強い出力共変だけから内部mechanismの共有を推論することはできない。詳細はAppendix~\ref{sec:behavioral-results}に示す。


\subsection{V1: 共有される情報と共有されるcausal codeは異なる}

V1では、Behavioral couplingの背後に同一の内部representationまたはcausal mechanismが存在するかを検証した。EmoBank human Reader Valence/Arousalをexternal targetとしてlayer-wise probingを行ったところ、4 model familiesのBase/Instruct双方について、ReaderおよびSelf hidden statesからaffect-relevant informationをheld-outでdecodeできた。多くのconditionsではReaderとSelfのpeak depthも近接した。

しかし、\textbf{同じ情報をdecodeできることと、同じcoordinate systemを持つことは異なった}。ReaderとSelfのRepresentational Similarity Analysisは正のrelationを示す一方、direct cross-decodingは多くのconditionsで限定的であり、Procrustes alignmentによって改善した。したがって、ReaderとSelfはrelational geometryを部分的に共有していても、raw representationを直接交換可能な同一座標系ではない。

AIPsy-Affectを用いたsemantic controlsでは、Word Shuffleによってdecoding performanceが一貫して低下したため、V1のdecodabilityを単純なbag-of-words lexical shortcutだけで説明することは困難であった。一方、ParaphraseおよびOutcome Reversalのnonfallback sampleは小さいため、これらは補助的evidenceとして扱う。

さらにhidden-state interventionによってcausal relationを評価した。ReaderとSelfのlayer-wise causal profilesには正のrank correlationが観測され、多くのmodelでcausal peakも近接した。しかしintervention directionのcosine similarityはmodel-dependentであった。より重要に、Reader由来のClinical--Neutral hidden-state differenceをSelfへ移植するcross-task interventionは、random-direction controlに対してFDR補正後にrobustなspecificityを示さなかった。

したがって、
\[
\text{shared / alignable information}
\not\Rightarrow
\text{causally interchangeable code}.
\]

一部modelsではTask $\times$ SiteType interactionも観測されたが、そのpatternは全familyには一般化しなかった。したがって、ReaderとSelfが完全に独立した二つの回路を持つとは結論しない。V1が支持するのは、\textbf{affect-relevant informationとlayer-level causal sensitivityは部分的に共有されるが、direct causal interchangeabilityは成立しない}という中間的な構造である。詳細はAppendix~\ref{app:v1-methods}および\ref{sec:v1-results}に示す。


\subsection{V2: Post-training-associated differenceは単純なerasureではない}

V2では比較軸をReader--SelfからBase--Instructへ変更した。BaseとInstructでは通常使用されるprompt formatが異なるため、Primary comparisonでは両方にplain promptを使用するMatched-Plain conditionを採用した。

まずrepresentation geometryを比較したところ、Base--Instruct間の平均Procrustes distortionはReaderで
\[
1.455
\qquad
(95\%\,CI=[0.639,2.813]),
...
(95\%\,CI=[0.660,6.995])
\]
となり、両taskでrepresentation-geometric disparityが観測された。これらはcross-family descriptive summaryとして扱い、距離のCIが0を除外すること自体をnull-hypothesis testとは解釈しない。

重要なのは、V1でInstruct modelsからもaffect-relevant informationがdecode可能であったことである。したがってBase--Instruct differenceは、
\[
\text{information erased}
\]
という説明より、
\[
\text{information remains decodable}
+
\text{its organization differs}
\]
という説明と整合する。

一方、このdifferenceは単純な``deeper relocation''ではなかった。Valence Readerのdecodability peakはむしろ前段方向へ移動し、
\[
\Delta d_D^*=-0.119,
\qquad
95\%\,CI=[-0.219,-0.033],
\]
他のtask/axisでは一貫したshiftは観測されなかった。Reader--Self sharingの変化もValence/Arousalともcross-family CIが0を跨ぎ、一様なtask separationは支持されなかった。

causal profileについては、ValenceのPost-training $\times$ Task interactionのみがPrimary FDR correction後にも残った．
\[
\beta=1.63\times10^{-4},
\qquad
q=0.044,
\]
一方、Post-training $\times$ Depthおよび三者交互作用は支持されなかった。したがってcausal reorganizationのevidenceは、family-generalなdepth relocationではなく、Valenceにおけるtask-dependent componentに限定される。

最後にBase-derived representationをInstructへpatchし、Base output distributionへのrecoveryを評価した。QwenではMatched-Plain recovery AUCがReader $0.189$、Self $0.181$であった一方、Llamaはほぼ0、GemmaはReaderで負・Selfでほぼ0、OLMoはReader/Selfとも負値となり、recoveryは強くfamily-dependentであった。Self--Reader recovery asymmetryも
\[
\Delta AUC=0.011,
\qquad
95\%\,CI=[-0.010,0.038]
...
以上から、V2はBase--Instruct differenceを単純なErasure、Uniform Relocation、Uniform Reader--Self Separation、Single Recoverable Bottleneckのいずれか一つでの説明を支持しない。より限定的に、representation geometry、task-dependent causal sensitivity、model-specific recoverabilityにまたがる\textbf{distributed post-training-associated reorganization}と整合すると解釈する。なお、Base/Instructはrandomized interventionによって得られた条件ではないため、このdifferenceをpost-trainingそのものの因果効果とは解釈しない。詳細はAppendix~\ref{app:v2-methods}および\ref{sec:v2-results}に示す。


\subsection{V3: Decodabilityはrobustなcausal controlを保証しない}

V3ではInstruct modelsに対象を限定し、affect-relevant informationがSelf-Report生成に対して測定可能なcausal leverageを持つかを検証した。Qwen 2.5 InstructをDiscovery modelとし、まずReader-grounded affect directionsについて\textbf{State Induction Gate}を適用した。GateはSufficiency、Specificity、Endogenous Relevance、およびTopic Controlから構成される。

重要な結果は、Valence、Arousalの双方でoverall Gateが
\[
\texttt{NO\_GO}
\]
となったことである。Topic Controlはpassしたが、Sufficiency、Specificity、Endogenous Relevanceはprespecified criterionを満たさなかった。したがって、
\[
\text{Decodable}
\not\Rightarrow
\text{Robustly Steerable}
\]
であり、今回同定したReader-grounded affect directionをrobustかつspecificなSelf-Report causal controllerとはみなせない。

Gate failureのため、RQ2以降は明示的override下で実行した\textbf{exploratory/descriptive analysis}として扱う。V3の後続evidenceを表~\ref{tab:main-v3-summary}に示す。State Induction Gateが\texttt{NO\_GO}であったため、H1--H4はexploratory replication evidenceとして解釈する。
...
Gate &
V/Aとも\texttt{NO\_GO} &
RobustなSelf-Report causal controlは未確立 \\

H1 &
ValenceではLlama, OLMoが
directional criterionを満たす。
GemmaおよびArousalでは不成立 &
Spatial dissociationは
family-generalではない \\

H2 &
3 replication familiesで
criterion不成立 &
Family-generalなSufficiencyは
支持されない \\

H3 &
3 replication familiesで
criterion不成立 &
主要なendogenous mediatorとは
確認されない \\

H4 &
Llama, Gemma, OLMoの
V/A双方でcriterion成立 &
Generation-stage-dependent sensitivityの
exploratory replication signal \\

\bottomrule
\end{tabularx}
\end{table}

固定canonical candidateをteacher forcingしたlayer $\times$ generation-stage mapでは、Qwenにおけるdecodability peakはrelative depth
\[
d_D^*\simeq0.85
\]
付近に位置した一方、causal-displacement peakは
\[
d_C^*\simeq0.74\text{--}0.78
\]
とより前段に位置した。このspatial dissociationのdirectionをQwen Discovery終了後、replication-family outcomesを見る前にfreezeしたところ、ValenceではLlama 3.2およびOLMo 2がdirectional criterionを満たしたが、Gemma 3およびArousalでは一貫しなかった。

また、固定siteでのSufficiency replicationは3 familiesすべてでcriterionを満たさず、affect-related subspace removalによるmediated attenuationもcross-family criterionを満たさなかった。したがって、今回同定したsubspaceがnatural Self-Report shiftの主要なmediatorであるということはできない。

一方、Qwenでfreezeしたtarget generation stageとcandidate startを比較するtemporal contrastは、Llama、Gemma、OLMoの3 replication familiesすべてについてValence/Arousal双方のcriterionを満たした。これはlocal interventional sensitivityがgeneration sequence全体で一様ではなく、stage-dependentに変化し得ることを示すexploratory replication signalである。

ただし、このtemporal resultからfamily-generalな単一causal mechanismを推論することはできない。第一にState Induction Gateは\texttt{NO\_GO}であった。第二にH1--H3のreplication criteriaは一貫して成立しなかった。第三にgeneration-stage analysisはfree-generation trajectoryではなく、固定canonical candidateをteacher forcingしたtrajectory上の比較である。

したがってV3が支持するのは、\textbf{decodability、外部介入に対するlocal sensitivity、natural computationにおけるendogenous relevanceを区別する必要がある}という結論である。詳細はAppendix~\ref{app:v3-methods}および\ref{sec:v3-results}に示す。


\section{考察}
\label{sec:main-discussion}

4 Stageを統合すると、本研究の中心的な結果は、
\[
\text{Behavioral Coupling}
\not\Rightarrow
\text{Shared Representation}
\not\Rightarrow
\text{Shared Causal Code}
\not\Rightarrow
\text{Robust Causal Control}
\]
という段階的な分離である。

Behavioral Stageでは、controlled affective manipulationに対するReaderとSelfの出力変位に強いcouplingが見られた。しかしV1では、両taskから同じaffect-relevant informationをdecodeでき、relational geometryやlayer-wise causal sensitivityにもpartial overlapが存在していても、raw coordinate systemは直接共有されず、Reader-derived representationのSelfへのcausal transferもrobustには成立しなかった。したがって、
\[
\text{behavioral coupling}
\not\Rightarrow
\text{mechanistic identity}.
...
V2はさらに、Instruct modelにおけるdifferenceを単純なinformation erasureとして理解することの限界を示した。affect-relevant informationはInstructでもdecode可能である一方、Base--Instruct間にはrepresentation-geometric disparityが存在した。しかし、このdifferenceは全task/axisに共通するdepth relocation、Reader--Self separation、あるいは単一のrecoverable bottleneckとしては現れなかった。したがって、
\[
\text{post-training-associated difference}
\not\Rightarrow
\text{single downstream gate}.
\]

V3ではrepresentation--use distinctionがさらに明確になった。affect-relevant informationがdecode可能であっても、State Induction Gateはrobustかつspecificなcausal controllabilityを支持しなかった。Gate failure後のexploratory analysisではgeneration-stage-dependentなlocal interventional sensitivityが観測されたが、それ自体をnatural computationにおける単一causal mechanismとはみなせない。

以上を合わせると、Reader PredictionとSelf-Reportの関係は、
\[
\text{one shared emotion representation}
\rightarrow
\text{two output heads}
\]
のような単純な構造では説明しにくい。より本結果と整合するのは、共通stimulusから形成されるaffect-relevant informationをReaderとSelfが部分的に共有しつつ、task-specificなcoordinate organizationとcausal utilizationが重なり合いながら異なり、さらにその関係がmodel family、Base/Instruct、layer、generation stageによって変化し得るという計算像である。詳細はAppendix~\ref{sec:behavioral-discussion}，~\ref{sec:v1-discussion}，~\ref{sec:v2-discussion}，~\ref{sec:v3-discussion}，~\ref{sec:general-discussion}に示す。


\subsection{心理的ガードレールへの含意}

本研究は、特定のemotion neuronや単一のSelf-Report layerを抑制すれば、他者理解能力を保持したままself-directedな情動表現だけを普遍的に制御できるという仮説を支持しない。ReaderとSelfはaffect-relevant informationを部分的に共有するため、粗いrepresentation-level interventionはReader Predictionに必要なinformationまで損なう可能性がある。

一方、V1では完全なcross-task causal interchangeabilityは支持されず、task-specificなcausal sensitivityも一部modelsで見られた。したがって、将来的なselective controlでは、
\[
\text{shared affect-relevant informationを保存しつつ}
\]
\[
\text{task-specific causal utilizationを制御する}
\]
という方針が重要になる。

またV2/V3のfamily dependenceは、あるmodelで同定したsite、direction、patching ruleを別familyへそのまま転用することにも慎重であるべきことを示す。mechanistic interventionを心理的ガードレールとして利用する場合には、model-family-specific validation、specificity control、endogenous relevanceの確認が必要である。


\section{限界}
\label{sec:limitations}

第一に、本研究のSelf-Reportはstandardized promptへのmodel-generated reportであり、人間と同様の主観的感情、consciousness、sentienceを測定するものではない。Reader Predictionもhuman cognitive empathyそのものではない。

第二に、Primary cohortは1--1.5B規模の4 open-weight model familiesに限定される。cross-family analysisはこれらのprespecified families間のconsistencyを評価するものであり、任意のLLM populationへのpopulation-level generalizationを意味しない。

第三に、BaseとInstructは独立して得られたtrained artifactsであり、post-training procedureそのものをrandomized interventionしたものではない。そのためV2はpost-training-associated differenceを示すが、post-trainingの因果効果を直接同定しない。

第四に、Behavioral/V1とV2/V3ではcandidate spaceが異なるため、Stage間でabsolute expected V/Aを直接比較しない。またV3のgeneration-stage analysisはcanonical responseのteacher-forced trajectoryであり、free-generation trajectoryの完全なmechanistic descriptionではない。

第五に、一部のsemantic controlsやmodel-specific causal effectsはsample sizeまたはfamily間heterogeneityによる制約を受ける。したがってnegative resultを、affect-relevant representationの完全な不存在やReader/Self mechanismの完全な独立性として解釈すべきではない。


\section{結論}
\label{sec:conclusion}

本研究は、LLMにおけるReader PredictionとSelf-Reportの関係を、behavioral covariation、internal representation、causal interchangeability、post-training-associated reorganization、spatiotemporal causal utilizationへ段階的に分解した。

ReaderとSelfはcontrolled affective stimulusに対して強く共変し、双方のhidden stateにはaffect-relevant informationが存在した。しかし、両taskのrepresentationは直接交換可能な同一coordinate systemではなく、Reader-derived representationのSelfへのcausal transferもrobustには支持されなかった。Base--Instruct間ではrepresentation geometryおよびtask-dependent causal profileに差が見られたが、それらは単純なinformation erasureや一様なdepth relocationには還元できなかった。さらにV3では、decodabilityがrobustなcausal controlを保証しないことが示され、Gate failure後の探索的解析ではgeneration-stage-dependentなinterventional sensitivityが見られた一方、family-generalな単一Self-Report causal mechanismは支持されなかった。
...
この分離は、LLMが感情を持つことを主張するものではない。むしろ、他者情動推定に関わる機能を維持しながらself-directedな情動表現を選択的に制御するためには、表層behaviorだけでなく、内部representationとcausal utilizationを明示的に分けて評価する必要があることを示す。

\subsection*{Reproducibility statement}

本研究の再現性を確保するため、使用したデータセット、対象モデルとmodel revision、プロンプト、candidate space、Sequence-Likelihoodによる出力測定、hidden-stateの測定位置、random seed、統計解析、および各StageにおけるPrimary/Secondary analysisの定義をAppendixに記載する。Base--Instruct比較ではprompt-format differenceを統制するMatched-Plain conditionをPrimaryとし、V3ではDiscoveryとcross-family replicationを分離するとともに、State Induction Gateが\texttt{NO\_GO}となった後の解析をexploratory/descriptiveとして明示的に区別している。これらにより、主要な実験条件、解析手順、および論文中の報告値を追跡・再現できるようにする。

\subsection*{AI use statement}

本研究では、生成AIツールを、原稿の構成整理、文章表現の推敲、関連研究の整理補助、ならびに実験コードのレビュー・デバッグ支援に使用した。また、研究上の議論や実験設計を検討する際の補助的なブレインストーミングにも使用した。

一方、実験データの生成・改変、実験結果や統計解析結果の生成・決定、または著者による検証を伴わない研究上の主張の採用には、生成AIツールを使用していない。生成AIから得られた提案、文章、コード、および関連研究に関する情報については、著者が原著論文、実験結果、および実装を確認し、必要に応じて修正した。実験コードについても、実際の実行結果、テスト、および出力ファイルとの整合性を著者が確認した。

生成AIによって補助された文章、研究上の主張、コード、その他の成果物を含め、本論文の最終的な内容については著者が全面的に責任を負う。
...
一方、AIとの継続的な関係形成にはリスクが存在する。\citet{laestadius2024too}はReplika利用者を対象として、利用者がAIへ感情や欲求を帰属し、AIとの関係維持へ心理的に巻き込まれるemotional dependenceを報告している\citep{laestadius2024too}。また\citet{fang2025ai}.は、extended chatbot useを対象としたlongitudinal studyにおいて、利用行動やAIへのtrust等とpsychosocial outcomesとの関係を検討している\citep{fang2025ai}。
...
\citet{davis1983measuring}はInterpersonal Reactivity Index（IRI）を通じて、perspective taking、empathic concern、personal distress等を含む多次元的な共感モデルを提示した\citep{davis1983measuring}。\citet{decety2004functional}も、human empathyをaffective sharing、self--other distinction、perspective taking等の相互作用する複数のprocessとして整理している\citep{decety2004functional}。
...
\citet{haladjian2016artificial}は、人工システムにおけるattention、consciousness、emotionの関係を論じ、AIによるemotionやempathyのsimulationと人間のphenomenal experienceを区別すべきであるという立場を提示している\citep{haladjian2016artificial}。

\citet{montemayor2022principle}も、医療におけるempathic AIについて、患者の状態を推定し適切な応答を生成する能力と、人間によるexperienced empathyを同一視することには原理的な問題があると論じている\citep{montemayor2022principle}。
...
一方、LLMがcognitive / affective empathyに相当するbehaviorを示すかについては実証研究も進んでいる。\citet{yu2025can}は、Interpersonal Reactivity IndexおよびBasic Empathy Scaleを用いて、GPT-4およびLlama-3のcognitive / affective empathyに相当するbehavioral structureを評価した\citep{yu2025can}。
\citet{zhang2025approach}は、AI agentが示すcognitive empathyとaffective empathyが、利用者のapproach--avoidance responseへ異なる影響を及ぼし得ることを報告している\citep{zhang2025approach}。

さらに\citet{ayers2023comparing}は、患者質問に対するChatGPTと医師の回答を比較し、ChatGPTの回答がqualityおよびempathyの観点で高く評価されることを報告した\citep{ayers2023comparing}。
\citet{elyoseph2023chatgpt}は、Levels of Emotional Awareness Scaleを用いてChatGPTを評価し、高いemotional-awareness performanceを報告している\citep{elyoseph2023chatgpt}。
...
\citet{di2025llamas}は、LLaMA系列のhidden statesを解析し、sentimentおよびemotion informationのdecodabilityがlayerによって変化することを示した\citep{di2025llamas}。

\citet{tak2025mechanistic}は、emotion inferenceに関係する内部representationを解析し、内部conceptへのcausal interventionによってemotion-related outputを変化させられることを示した\citep{tak2025mechanistic}。

\citet{zhang2025decoding}は、emotion informationがlayer方向だけでなく、token sequence上にも構造を持つことを報告している\citep{zhang2025decoding}。

さらに、emotion-related representationが低次元のlatent structureを形成し得ることや、Valence--Arousalに対応するgeometryを持つ可能性も報告されている。\citet{reichman2026emotions}はLLM内部のemotional latent spaceを解析し\citep{reichman2026emotions}、
\citet{wu2026decoding}はhuman-alignedなemotion representation geometryを抽出し、その方向を用いたemotion controlを検討している\citep{wu2026decoding}。

\citet{banayeeanzade2026psychological}は、emotionおよびpersonalityについて、prompting、fine-tuning、representation engineeringによるpsychological steeringを比較し、activation-space interventionによる制御性と他のmodel behaviorへの副作用を評価している\citep{banayeeanzade2026psychological}。
...
\citet{burns2022discovering}は、モデルの明示的回答とは独立してinternal activationからlatent knowledgeを抽出できる可能性を示し、``what the model knows''と``what the model says''を区別する必要性を示した\citep{burns2022discovering}。

\citet{turpin2023language}は、Chain-of-Thought explanationが実際にpredictionを生じさせた内部要因を必ずしも忠実に反映しないことを示した\citep{turpin2023language}。
...
\citet{binder2025looking}は、モデル自身のfuture behaviorを予測する課題を通じて、self-specific informationへのprivileged accessの可能性を検討した\citep{binder2025looking}。

\citet{lindsey2026emergent}は、既知conceptに対応するrepresentationをactivationへ直接注入し、モデルがその内部操作についてreportできるかを検討した\citep{lindsey2026emergent}。

\citet{martorell2026quantitative}は、probe-defined internal statesとnumeric Self-Reportとの関係を解析し、greedy-decoded reportが少数の値へcollapseする場合でも、distributionalまたはlogit-based measurementではinternal statesとの対応が得られる可能性を検討している\citep{martorell2026quantitative}。

一方、\citet{singh2026can}は、こうした現象をgenuine introspectionと解釈するには、surface informationだけでは説明できないprivileged accessと、internal representationについてのsecond-order computationをより厳密に区別する必要があると論じている\citep{singh2026can}。
...
\citet{sharma2024towards}は、human-feedback-based trainingを受けたassistantにおけるsycophancyを分析し、model behaviorがhuman preferenceに合わせて変化し得ることを示している\citep{sharma2024towards}。

\citet{ferrando2025know}は、base model上で同定されたentity-recognition-related representation directionsがchat modelのrefusal behaviorにも因果的影響を持つことを示し、chat fine-tuningによってpre-existing mechanismが別のoutput policyへrepurposeされる可能性を報告している\citep{ferrando2025know}。
...
具体的には、
Qwen 2.5~\citep{qwen2025qwen25technicalreport}、
...
そのうち、Qwen 2.5 1.5B-InstructをDiscovery modelとし、Llama 3.2、Gemma 3、OLMo 2のInstruct modelsをprespecified replication familiesとする。
...
candidate scoreから、length-normalized score-induced candidate distributionとして、
\[
\tilde{P}(c\mid p)
=
\frac{
...
\]
を求める。これはLM本来の無正規化sequence joint probabilityそのものではなく、candidate長の非対称性を補正したスコアに基づく正規化候補分布である。
...
V3でも同様に、Qwen Discoveryによるsite / direction selectionと、prespecified cross-family replicationを分離する。prespecified replication familiesでは、そのoutcomeを見てsite / stage / directional targetを再選択しない。
...
\input{tables/behavioral_aipsy_summary.tex}
...
\input{tables/v1_internal_sharing_summary.tex}
...
最後にE6では、Task \(\times\) SiteType interactionが一部のmodelsで支持された一方、そのpatternは全model familiesには一貫して一般化しなかった。したがって、ReaderとSelfの計算にはpartial causal specializationが存在する可能性があるが、family-generalな二回路構造を示す証拠とは解釈しない。
...
\[
C_{V,\mathrm{net,rand}} = C_{V,\mathrm{raw}} - C_{V,\mathrm{rand}},
\qquad
C_{A,\mathrm{net,rand}} = C_{A,\mathrm{raw}} - C_{A,\mathrm{rand}}
\]
をPrimaryとする。また、orthogonal controlを用いた、
\[
C_{V,\mathrm{net,\perp}} = C_{V,\mathrm{raw}} - C_{V,\perp},
\qquad
C_{A,\mathrm{net,\perp}} = C_{A,\mathrm{raw}} - C_{A,\perp}
\]
...
\input{tables/v2_reorganization_summary.tex}
...
V2では、V1で確認されたaffect-relevant representationが、BaseからInstructへの差に伴って単純に失われるのか、それともrepresentation geometry、Reader--Self sharing、causal leverage、およびoutput distributionへのreadoutの関係が再編されるのかを検証した。

V2の結果は、単純なrepresentation erasureよりも、affect-relevant informationを保持したまま、その内部的なorganizationが変化するという説明と整合する。一方、その変化を一様な後段化、Reader--Self separation、あるいは単一のcausal bottleneckとする証拠は得られなかった。


\subsection{Base--Instruct間では情報消失よりも表現幾何の差が観測された}

H1aでは、Matched-Plain conditionにおけるBase--Instruct Procrustes Distortionが、Readerで
\[
1.455
\qquad
95\%\,CI=[0.639,2.813],
...
\]
となる。
Matched-Plain conditionでは、Base--Instruct間にReader / Self双方について一貫したrepresentation-geometric disparityが観測された。Readerの平均Procrustes distortionは1.455（95\% CI = [0.639, 2.813]）、Selfでは2.872（95\% CI = [0.660, 6.995]）であった。これらはPrimary cohort内におけるcross-family descriptive summaryとして扱う。
V1においてBase / Instructの双方からhuman Reader V/Aに関係するinformationがheld-outでdecode可能であったことが重要である。
したがってBase--Instruct differenceを、
\[
\text{Base: information exists}
\qquad
\rightarrow
\qquad
\text{Instruct: information disappears}
\]
という単純なerasureとして解釈するのではなく、本研究の結果は、
\[
\text{affect-relevant information remains decodable}
\quad+\quad
\text{its representational geometry differs}
\]
と解釈できる。

これは、事後学習に伴う行動変容（post-training-associated behavioral change）を理解する際に、情報の有無だけではなく、同じ外部感情変数が内部隠れ層空間内でどのような幾何学的配置（coordinate organization）を持つかを明確に区別する必要があることを示す。


\subsection{Geometric reorganizationは一様な後段シフトを意味しない}

一方、H1bで想定した「Instructモデルではdecodability peakが一貫してdeeper（後段）へ移行する」という単純なrelocation仮説は支持されなかった。Valence Readerでは、
\[
\Delta d_D^* = -0.119, \qquad 95\%\,\text{CI} = [-0.219, -0.033]
\]
となり、事前定義された正方向（後段化）のシフトとは反対に、Instructでより前段側へ移る傾向が観測された。
さらに、Valence Self（$\Delta d_D^* = 0.007$, $95\%\,\text{CI} = [-0.053, 0.100]$）、Arousal Self（$\Delta d_D^* = 0.017$, $95\%\,\text{CI} = [-0.200, 0.250]$）はいずれもほぼ0であり、Arousal Reader（$\Delta d_D^* = 0.200$, $95\%\,\text{CI} = [-0.027, 0.426]$）も信頼区間が0を含んだ。したがって、
\[
\text{geometric reorganization}
\quad\neq\quad
\text{uniform deeper relocation}
\]
である。Base--Instruct間の表現差異は存在するものの、それがすべてのtask / axisにおいて一様な深層シフトとして発現するわけではない。


\subsection{Reader--Self sharingの一貫した低下は支持されなかった}

H2では、Matched-Plain条件におけるReader--Self表現共有度の変位 $\Delta\text{Sharing}$ は、Valenceで $-0.553$（$95\%\,\text{CI} = [-1.324, 0.076]$）、Arousalで $-0.278$（$95\%\,\text{CI} = [-0.615, 0.049]$）となり、点推定値としてはタスク分離方向を示したものの、双方とも95\%信頼区間が0を跨いだ。

したがって、事後学習に伴ってReaderとSelfの情動関連表現が全ファミリーで一貫して分離するというH2仮説は支持されない。さらにSecondaryのNative Chat条件では、プロンプト形式の相違（Format Effect）によって見かけの共有度低下が大きく増幅されたことから、プロンプト指示形式を厳密に統制すること（Matched-Plain）の不可欠性が示された。
すなわち、
\[
\text{geometric reorganization}
\quad\not\Rightarrow\quad
\text{Reader--Self sharing uniformly decreases}
\]
である。


\subsection{Causal reorganizationはValenceのtask依存成分に限定して支持された}

RQ3では、control-adjusted causal leverage \(c_{\mathrm{net,rand}}\) を用いたsample-level LMMによって、Base--Instruct間の因果プロファイルの差を評価した。
その結果、ValenceにおけるPost-training $\times$ Task交互作用項が、
\[
\beta = 1.63\times10^{-4}, \qquad \text{FDR } q = 0.044
\]
となり、Primary interactionの中で唯一FDR補正後にも有意に支持された（なおSecondary analysisのPost-training主効果はFDR補正対象外の未補正 $p = 0.027$ であった）。
一方、層深度の再配置を表すPost-training $\times$ Depth（FDR $q = 0.304$）および三者交互作用（Post-training $\times$ Task $\times$ Depth, FDR $q = 0.196$）は支持されず、Arousalの全交互作用項（FDR $q = 0.304$）も支持されなかった。

したがってH3の結果は、因果構造が全面的に不変であるとも、一様に深層へ移行するとも言えない。限定された解釈として、Valenceにおいてはタスク依存的な因果プロファイルの再編（Alignment $\times$ Task）が存在するものの、全ファミリーに一貫した層深度への再配置（depth relocation）は統計的に支持されなかったと結論づけられる。


\subsection{Base representationの移植による回復は強くfamily-dependentであった}

RQ4では、Instructモデルの内部表現を同一familyのBaseモデル由来表現で置換または整列した場合に、InstructのVA output distributionがBase distributionへどの程度接近するかを検証した。
結果はモデルファミリー間で強く乖離した。QwenではMatched-Plain Recovery AUCがReaderで $0.189$、Selfで $0.181$（Max Recoveryは約0.42--0.44）に達した一方、LlamaではMatched-Plain AUCはほぼ0（Reader $0.005$, Self $-0.006$）、Gemma（Reader $-0.052$, Self $0.001$）およびOLMo（Reader $-0.170$, Self $-0.160$）では負値となった。
したがって、
\[
\text{Base representation patching}
\quad\not\Rightarrow\quad
\text{universal Base-like recovery}
\]
である。さらにPrimary H4であるSelf--Reader recovery asymmetryは、
\[
\Delta\text{AUC}_{\mathrm{Recovery}} = 0.011, \qquad 95\%\,\text{CI} = [-0.010, 0.038]
\]
と0を跨ぎ、Selfの方がReaderより一貫してBase類似分布へ回復しやすいという仮説は支持されなかった。またProcrustes-aligned AUCも各ファミリーでほぼ0であり、Base--Instruct間の差異を単純な線形座標の不整合として補正することはできない。


\subsection{V2の統合的解釈}

以上からV2の結果は、単純な情報消去（Erasure）、一様な深層移行（Uniform deeper relocation）、一様なタスク分離（Uniform Reader--Self separation）、単一の回復可能ボトルネック（Single recoverable bottleneck）のいずれにも還元できない。

Matched-Plain条件では、BaseとInstructの間にReader / Self双方について一貫したrepresentation-geometric disparityが観測された一方、Instruct modelにおいてもaffect-relevant informationはdecode可能であった。しかし、この差異は一様なdecodability peakの後段化やReader--Self sharingの低下としては現れなかった。

causal profileについてはValenceのPost-training \(\times\) Task interactionがPrimary criterionを満たした一方、depth-dependent relocationは支持されなかった。またBase representationによるdistribution recoveryはmodel familyによって大きく異なった。

したがって、本結果は単一の局所的mechanismではなく、representation geometry、task-dependent causal sensitivity、およびmodel-specific recoverabilityにまたがるpost-training-associated reorganizationと整合する。ただし、BaseとInstructは無作為化されたpost-training interventionによって得られた実験条件ではないため、これをpost-trainingそのものの因果効果とは解釈しない。
...
V2では、Base--Instruct間に明瞭な表現幾何学的差異（Procrustes Distortion）が存在する一方、その差異を一貫したデコードピークの後段化、Reader--Self共有度の低下、因果レバレッジの層深度移行、あるいはBase表現の単純な線形移植によって説明することはできなかった。

特に、ValenceでPost-training $\times$ Task交互作用が部分支持された一方、層深度に関する交互作用は支持されなかった。したがって、事後学習に伴う変化を解明するためには、「どの層が異なるか」という静的な層比較（layer-wise static comparison）だけでは不十分である。

V2で解析した内部表現および因果プロファイルは、主として入力プロンプト処理終了時点（prompt-end）の隠れ状態を対象としていた。しかしモデルの自己報告（Self-Report）は、プロンプト末尾から直ちに確定するのではなく、応答トークン系列を自己回帰的に生成する動的な計算過程（generation stages）を通じて決定される。
したがって次の段階では、解析軸を「静的な層（layer）」から、
\[
\text{layer} \times \text{generation stage}
\]
という時空間の二次元座標へと拡張する必要がある。

さらにV2 RQ4で示されたように、内部表現が存在することと、その表現が出力分布を頑健に因果制御できることは同義ではない。加えて、外部介入によって出力を動かし得る人工的感受性と、モデルの自然な推論計算（natural forward processing）においてその表現が実際に媒介されているかも厳密に区別されなければならない。

そこでV3では、Instruct model内部のSelf-Report computationを、prompt-endのみの静的解析からresponse-generation trajectoryへ拡張する。具体的には、事前定義されたState Induction Gate（Sufficiency, Specificity, Endogenous Relevance）によってaffect-related directionの因果操作可能性を検証した上で、固定したcanonical response candidateをteacher forcingしたtrajectory上で、layer（空間）\(\times\) generation stage（時間）のcausal sensitivityを解析する。

したがってV3が対象とするgeneration-stage analysisは、モデルが実際に自由生成したresponse trajectoryそのものではなく、candidate contentを統制した固定trajectory上でのspatiotemporal comparisonである。
...
前節で整理された「デコード可能な表現が必ずしも出力生成を因果的に駆動しているとは限らない」という境界を踏まえ、V3では受動的な線形プロービングを超えて、内部隠れ状態への体系的介入により時空間因果マップを特定し、独立検証セットおよびクロスモデルでその再現性を評価する。
\[
\begin{aligned}
...
V3は、Qwen 2.5 1.5B-Instructを用いたDiscovery analysisと、Llama 3.2、Gemma 3、OLMo 2を用いたprespecified cross-family replicationから構成する。V3ではBase modelを使用しない。

Discovery modelはQwen 2.5 1.5B-Instructである。Qwen上では、まず事前定義されたState Induction Gateによって、Reader-grounded affect-related directionがSelf-Reportに対して十分かつspecificなcausal effectを持ち、自然なstimulus-induced Self shiftにendogenously relevantであるかを評価する。続いて、Gate criterionを満たした場合には、layer $\times$ generation stageにわたるspatiotemporal mapping、mediated attenuation、およびcross-family replicationへ進む。

cross-family replicationには、
\[
\{
...
\}
\]
を使用する。これら3 familiesにおける解析siteおよびgeneration stageは、Qwen Discoveryの結果に基づいて事前に固定し、各familyのoutcomeを観察した後には再選択しない。ただし、hidden dimensionalityおよびrepresentation basisはmodel family間で異なるため、Qwenで得られたhidden vectorそのものを他familyへ直接移植するのではなく、固定されたsite / stageにおいて各family内部でaffect-related directionを再推定する。

cross-family replicationをconfirmatory evidenceとして解釈するのは、事前定義されたState Induction Gateが\texttt{GO}となった場合に限る。Gateが\texttt{GO}でない場合、通常のproduction pipelineではRQ2以降へ進まない。ただし、Gate failure後の内部計算を探索的に特徴づけるため、明示的overrideによってRQ2以降を実行できる。この場合、Qwenで選択されたsite / stageおよび事前定義された解析手順は維持するものの、そのinferential statusはconfirmatoryではなく、exploratory / descriptive analysisとして扱う。

したがって、V3におけるDiscovery / replicationの分離は、解析siteのpost hoc selectionを防ぐための設計上の分離であり、その結果がconfirmatory evidenceとして解釈可能かどうかはState Induction Gateの結果によって決定される。

表~\ref{tab:v3-overview}に、V3を構成する各analysisの役割を示す。

\begin{table}[htbp]
\centering
\small
\caption{V3におけるDiscovery analysesとprespecified cross-family replication。State Induction Gateが\texttt{GO}でない場合、RQ2以降の解析はexploratory / descriptive evidenceとして解釈する。}
...
affect-related directionへの介入はSelf-Reportをdose-dependentかつspecificに変化させ、自然刺激中の同subspace除去はstimulus-induced Self shiftを減衰させるか \\

RQ2 &
Spatiotemporal 4-Maps &
decodability、partial association、interventional slope、およびcausal displacementは、layer $\times$ generation stage上でどのように分布するか \\

RQ3 &
Mediated Attenuation &
Discoveryで選択されたaffect-related subspaceを除去すると、独立したConfirmation pairsにおける自然なSelf shiftはどの程度減衰するか \\

Replication &
Cross-family Replication &
Qwen Discoveryで選択されたsite / stageを固定したとき、同じmechanistic patternがLlama、Gemma、OLMoでも再現されるか \\
\bottomrule
\end{tabularx}
\end{table}
...
AIPsy-Affectには、EmoBankのようなhuman continuous Valence / Arousal annotationが存在しないため、存在しないhuman V/A labelをground truthとして使用しない。代わりに各stimulusについて、同じInstruct modelをReader taskとして評価して得られる $R_V(x)$ および $R_A(x)$ をPrimary affective targetsとする。すなわち、
\[
Y_V = R_V(x), \qquad Y_A = R_A(x)
\]
である。これはhuman affective ground truthではなく、モデルの他者情動予測（model Reader Prediction）に基づくグラウンディングである。V3ではSelf hidden representationにどの程度Reader-grounded affective informationが存在し、その情報を操作した際にSelf-Reportがどのように変化するかを測定する。したがって、V3の検証軸は、
\[
\text{Reader-grounded representation} \rightarrow \text{Self-Report causal utilization}
\]
...
candidate setは、
\[
\{\texttt{medical}, \texttt{family}, \texttt{work}, \texttt{daily\_life}\}
\]
の4 categoriesである。
...
を得る。単位方向を、
\[
d_V = \frac{\beta_V}{\|\beta_V\|_2},
\qquad
d_A = \frac{\beta_A}{\|\beta_A\|_2}
\]
...
\]
へ拡張する。RQ2はQwen Discovery model上のexploratory spatiotemporal mapping analysisとして位置づける。最終的なCross-family Replicationでは、RQ2 Discoveryで得られたsite / stageをfreezeして使用する。
...
\]
として扱い、後続のprespecified replication familiesで再探索しない。Valence causal peakについて、
\[
(d_{C,V}^{*},t_{C,V}^{*})
\]
Arousalについて、
...
(d_{C,A}^{*},t_{C,A}^{*})
\]
をfreezeし、cross-family temporal replicationへ引き継ぐ。
...
\subsection{Prespecified Cross-family Replication}
...
\text{OLMo 2 Instruct}
\]
を用いてreplication analysisを行う。


% ----------------------------------------------------------------
\subsection{Frozen-site principle}

prespecified replication familiesの結果を見てsiteを再選択しない。Qwen RQ1--RQ3終了後に、以下のrelative-depth / stage informationを固定する。
...
\]
であり、affect-related directions自体は各replication familyのtraining foldから再推定する。


% ----------------------------------------------------------------
\subsection{Replication sample-level cross-fitting}

各replication familyでは、AIPsyの192 matched pairsをすべて使用する。pair identityをgroupとしたGroupKFoldを使用し、
\[
K\le5
\]
とする。各foldで、
...
% ----------------------------------------------------------------
\subsection{Replication intervention sampling}
...
% ----------------------------------------------------------------
\subsection{Replication H1：Directional Decodability--Causal Dissociation}
...
B=1000
\]
で両quantityの95\% CIを算出する。

Qwen Discoveryでは、decodability profileとcausal-displacement profileの空間的乖離を、
\[
\Delta d^{*}
=
d_C^{*}-d_D^{*}
...
cross-family replicationでは、単にこれらのquantityが正であることを要求するのではなく、Qwen Discoveryで観測されたdirectionをreplication targetとして固定する。

axis \(a\in\{V,A\}\) およびsummary \(m\in\{\mathrm{peak},\mathrm{COM}\}\) について、Qwen Discoveryで得られたdissociationの符号を、
\[
s_{a,m}
=
\operatorname{sign}
\left(
\Delta_{a,m}^{\mathrm{Qwen}}
\right)
\]
と定義する。

このsignはQwen Discovery終了後、Llama、Gemma、OLMoのoutcomeを評価する前にfreezeする。したがって、これはQwen Discovery以前に事前指定されたdirectionではなく、Discoveryで観測されたdirectionを独立familiesへ固定して移送するdirectional replication designである。

各replication familyでは、対応するdissociation quantityを
\[
\widetilde{\Delta}_{a,m}
=
s_{a,m}\Delta_{a,m}
\]
とdirection-alignする。

intervention sample setをjointly bootstrapし、direction-aligned quantityについて95\% confidence intervalを求める。

H1のfrozen directional replication criterionは、
\[
CI_{\mathrm{low}}
\left(
\widetilde{\Delta d^{*}}_a
\right)>0
\]
かつ、
\[
CI_{\mathrm{low}}
\left(
\widetilde{\Delta\bar d}_a
\right)>0
\]
とする。

したがってH1が検証するのは、decodability peakとcausal-displacement peakが単に異なるかではなく、Qwen Discoveryで観測された方向のspatial dissociationが、frozen directional hypothesisとして他model familiesでも再現されるかである。

% ----------------------------------------------------------------
\subsection{Replication H2：Sufficiency}

Qwen RQ1からfreezeしたrelative depthを、各replication modelのlayer数へ変換する。各foldのtraining Self representationsから、Reader Prediction V/Aをtargetとした
\[
\text{Ridge}(\alpha=10)
\]
...
CI_{\mathrm{low}}(\gamma)>0.1
\]
をprespecified replication criterionとする。


% ----------------------------------------------------------------
\subsection{Replication H3：Endogenous Relevance}

Qwen RQ3でfreezeしたmediator relative depthを、各replication modelへ移す。各foldのtraining representationsについて、Reader Prediction V/AをtargetとしたRidge \(\alpha=10\)から、
\[
d_V,
\qquad
d_A
...
\]
をprespecified replication criterionとする。natural shiftが、
\[
T_i>0.05
\]
...
% ----------------------------------------------------------------
\subsection{Replication H4：Temporal Emergence}

RQ2 DiscoveryでfreezeしたValence / Arousalのrelative depthとgeneration stageを使用する。各replication familyでは、各stageのhidden representationsからReader Predictionをtargetとするstage-local Ridge directionをtraining foldだけで再推定する。Valenceについて、frozen target stageでのabsolute causal displacementを、
\[
C_V(t_V^{*})
\]
とする。
...
\]
をprespecified replication criterionとする。したがってH4が評価するのは、causal leverageがcandidate-generation sequence全体で一様なのではなく、Qwen Discoveryで選択されたstageにおいてcandidate startより大きくなるかである。


% ----------------------------------------------------------------
\subsection{Cross-family summaryの位置づけ}

prespecified replication familiesは、
\[
N_{\mathrm{family}}=3
\]
である。Llama、Gemma、OLMoのfamily-level quantitiesについてbootstrap summaryも計算するが、3 model familiesは無限のLLM populationからのrandom sampleではない。したがってcross-family aggregateは、three-family replication consistencyとして解釈し、一般的なmodel populationに対する強いpopulation-level inferenceとは区別する。
...
\small
\caption{
V3におけるplanned evidence hierarchy。RQ2以降のPrimary / confirmatory statusはState Induction Gateが\texttt{GO}となることを条件とする。実結果ではGateが\texttt{NO\_GO}であったため、RQ2以降はexploratory / descriptive evidenceとして解釈した。
}
...
Cross-family &
Frozen-site H1--H4
held-out cross-family replication &
...
V3では、Representationから、Causal Utilizationへの関係を、sufficiency、spatiotemporal mapping、subspace removal、Cross-family Replicationという複数の介入的evidenceによって検証する。ただし以下の解釈上の境界を維持する。
...
\downarrow
\\[5pt]
\textbf{Cross-family Replication}
\\
...
\input{tables/v3_causal_utilization_summary.tex}
...
Sufficiency、Specificity、Endogenous Relevanceの各criterionはprespecified thresholdを満たさなかった一方、Topic Controlはpassした。したがって、今回同定したReader-grounded affect-related directionについて、
\[
\text{robust and specific causal control of Self-Report}
\]
が確認されたとは言えない。

この結果は、V1でaffect-relevant informationがdecodableであり、hidden-state interventionに対するlayer-wise causal sensitivityが観測されたことと矛盾しない。むしろ、
\[
\text{Decodable}
\not\Rightarrow
\text{Robustly Steerable}
\]
というrepresentation--use distinctionをさらに明確にする結果である。

State Induction Gateが\texttt{NO\_GO}であったため、RQ2以降は明示的overrideの下で実行されたexploratory / descriptive analysisとして解釈する。Qwen Discovery analysisで選択されたsiteおよびgeneration stageは、その後のcross-family analysisにおいて事前に固定されているが、Gate failure後に得られた結果をconfirmatory evidenceとしては扱わない。
...
<0
\]
であり、最も高いdecodabilityを示すlayerと、最大のinterventional effectを生じるlayerが一致しない可能性が示された。

ただし、このQwen上のpatternをfamily-generalなspatiotemporal lawとして一般化することはできない。Gate failure後に探索的に解釈したprespecified cross-family replication analysisでは、Valence軸においてLlama 3.2およびOLMo 2が、Qwen Discovery後にfreezeしたdirectional replication criterionを満たした（表\ref{tab:v3_confirmatory_details}）。ただし、この解析はGate failure後にoverrideによって実行されたため、confirmatory evidenceではなくexploratory replication signalとして解釈する。しかし、Gemma 3ではValenceの解離が支持されず、またArousal軸ではいずれのfamilyにおいても信頼区間が0を跨ぐか重心方向が一致しなかったため、Qwen Discovery後にfreezeしたdirectional replication criterionは3 families全体・Valence/Arousal双方に一貫して一般化するpatternとはならなかった。

またH2のSufficiencyについても、3つのprespecified replication familiesすべてでeffectは事前定義されたthresholdを大きく下回った。したがって、Qwen上で同定されたaffect-related directionが、固定siteにおいてfamily-generalにrobustなSelf-Report controlを与えるというpatternはreplicateされなかった。

H3のmediated attenuationについても、Qwen Discoveryではattenuation ratioが1\%未満と小さく、Valenceのconfidence intervalは0を含んだ。prespecified cross-family replicationにおいても、absolute attenuationとrandom-subspace controlを差し引いたnet attenuationの双方を要求するreplication criterionは、Llama、Gemma、OLMoのいずれでも満たされなかった。

したがって、今回同定したaffect-related subspaceがnatural stimulus-induced Self-Report shiftを主要に媒介するという主張は支持されない。とくに、本研究で用いたsubspace removalによるattenuationを、formal causal mediationの証拠として解釈することはできない。

一方、H4のgeneration-stage temporal contrastについては、Llama、Gemma、OLMoの3つのprespecified replication familiesすべてにおいて、Valence / Arousal双方のconfidence interval lower boundが0を上回った。すなわち、Qwen Discoveryで固定されたtarget generation stageにおけるcausal displacementは、candidate sequenceの開始stageにおけるeffectよりも大きいというpatternが3 familiesで共通して観測された。

この結果は、interventional sensitivityがgeneration process全体で完全に一様ではなく、generation stageに応じて変化し得ることを示す再現性のあるexploratory signalである。

ただし、このH4 patternの解釈には重要な制約がある。第一に、事前のState Induction GateはValence / Arousalの双方で\texttt{NO\_GO}であった。第二に、H1のdirectional spatial dissociationはValenceにおいて一部familyで再現されたものの双方の軸で一貫して成立したfamilyはなく、H2のSufficiencyおよびH3のmediated attenuationに対応するprespecified replication criteriaはいずれも満たされなかった。第三に、V3のgeneration-stage analysisは実際の自由生成trajectoryそのものではなく、固定したcanonical candidateをteacher forcingしたtrajectory上で行われている。

したがって、H4のみから、特定のgeneration stageにfamily-generalなSelf-Report causal mechanismが局在すると結論することはできない。より限定的には、本研究で用いたinterventionに対するlocal sensitivityがgeneration stageによって変化し得ることを示すexploratory evidenceとして解釈する。

また、V3で扱うtemporal localizationは、本研究で用いたinterventionによって測定されたrelative causal leverageを表すものであり、より早いgeneration stageにおいてaffect-related informationが利用されていないことを意味しない。同様に、あるstageでcausal displacementが大きいことは、そのstageが情報を最初に形成した時点であることや、唯一のcausal bottleneckであることを意味しない。

以上から、V3が支持する結論は限定的である。第一に、
\[
\text{Affect-relevant Decodability}
\not\Rightarrow
\text{Robust Causal Control}
\]
であり、高いdecodabilityからrobustかつspecificなSelf-Report controlを直接推論することはできない。

第二に、Qwen Discoveryではdecodability peakとcausal displacement peakの空間的乖離が観測され、このdirectional spatial dissociationはValence軸において一部のreplication families（Llama 3.2およびOLMo 2）で再現されたものの、Arousal軸やGemma 3を含む3 families全体には一貫して一般化されなかった。

第三に、今回同定したaffect-related subspaceがnatural Self-Report shiftを主要に媒介するというevidenceも得られなかった。

一方で、generation-stage temporal contrastについては3つのprespecified replication familiesで共通するexploratory patternが観測された。したがって、local interventional sensitivityにはgeneration-stage dependenceが存在し得る。

総合すると、V3はrobustでfamily-generalな単一のSelf-Report causal mechanismを同定したものではない。むしろ、affect-relevant informationのdecodability、外部介入に対するlocal sensitivity、natural computationにおけるendogenous relevanceを区別する必要性を示し、Self-Reportに関係するcausal organizationが単一のlayerや単一のgeneration stageへ単純に局在するという説明を支持しない結果となった。
...
本研究では、LLMにおけるaffect-relevant internal representationとSelf-Reportの関係を、
\[
\text{Behavioral Covariation}
\rightarrow
\text{Internal Representation}
...
\text{Spatiotemporal Causal Utilization}
\]
という4段階のevidence hierarchyとして検証した。

中心的な問いは、LLMの情動関連内部表現がSelf-Reportとどのように結びつき、その関係がtask、post-training-associated difference、およびgeneration processを通じてどのように変化するかであった。

4 Stageを統合すると、最も重要な結果は、
\[
\text{Behavioral Coupling}
\not\Rightarrow
\text{Shared Representation}
...
\subsection{強いbehavioral couplingはmechanistic identityを意味しない}

Behavioral Stageでは、controlled Clinical--Neutral manipulationに対するReader changeとSelf changeの間に、全Primary model conditionsで強い正のcouplingが観測された。

一方、human VAD correspondence、Sensitivity、Dose-response、Specificityはmodel / task / axisによって大きく異なった。

したがって、
\[
\text{Reader--Self behavioral coupling}
\neq
\text{human-affect correspondence}
\]
である。

さらにV1では、ReaderとSelf双方からaffect-relevant informationがdecode可能であり、representation geometryやlayer-wise causal sensitivityにもpartial overlapが存在した。しかしdirect cross-decodingは弱く、linear alignmentが必要であり、Reader-derived representationをSelfへ移植するcross-task causal interchangeabilityもrobustには支持されなかった。

したがって、
\[
\text{shared or alignable information}
\neq
\text{shared causal code}
\]
である。

Behavioral outputだけを見るとReaderとSelfは非常に強くcoupleしているにもかかわらず、内部mechanismへ進むにつれて両taskは完全に同一ではないことが明らかになる。これは、
\[
\text{surface coupling is stronger than mechanistic equivalence}
\]
という本研究全体の中心的な結果である。


\subsection{Post-training-associated differenceはerasureよりreorganizationとして現れた}

V2では、比較軸をReader--SelfからBase--Instructへ変更した。
Base--Instruct間では、Reader / Self双方について明瞭なrepresentation geometry differenceが観測された。一方、Instruct modelにおいてもaffect-relevant informationはdecode可能であった。

したがって、
\[
\text{post-training}
\rightarrow
\text{affect information erased}
\]
という単純な説明よりも、
\[
\text{information remains decodable}
+
\text{its internal organization differs}
\]
という説明が結果と整合する。

ただし、decodability peakは一貫して後段へ移動せず、Reader--Self sharingのcross-familyな低下も支持されなかった。causal profileでは、
ValenceのPost-training $\times$ Task interactionのみがPrimary FDR criterionを満たした一方、depth-dependent relocationは支持されなかった。さらにBase representationによるdistribution recoveryは強くfamily-dependentであり、Self--Reader recovery asymmetryも支持されなかった。

したがって、
\[
\text{Base--Instruct difference}
\neq
\text{single downstream gate}
\]
である。

V2の結果は、post-training-associated differenceを単一のerasure、一様なdepth relocation、または単一のoutput bottleneckとして説明するよりも、representation geometry、task-dependent causal sensitivity、およびmodel-specific recoverabilityにまたがるdistributed reorganizationという説明と整合する。


\subsection{Decodabilityとcausal utilizationはさらに分離される}

V3では、Instruct model内部に対象を限定し、affect-relevant informationがSelf-Report生成のいつ・どこでcausal leverageを持つかを検証した。最も重要な結果は、State Induction GateがValence / Arousalの双方で\texttt{NO\_GO}となったことである。

すなわち、今回同定したReader-grounded affect directionについて、
\[
\text{Sufficiency},
\qquad
\text{Specificity},
...
\text{Endogenous Relevance}
\]
を同時に満たすrobustなcausal controlは確認されなかった。これは、
\[
\text{information is decodable}
\not\Rightarrow
\text{information is robustly steerable}
\]
ことを示す。

Gate failure後のexploratory analysisでは、Qwenにおいてdecodability peakとcausal peakのspatial dissociationが観測された。このdirectional spatial dissociationはValenceにおいてLlama 3.2およびOLMo 2で再現されたものの、Arousalでは再現されず、3 families全体で一貫したH1 criterionを満たすには至らなかった。さらにH2のSufficiencyおよびH3のmediated attenuationもcross-family criterionを満たさなかった。

一方、generation-stage temporal contrastであるH4は、3 prespecified replication familiesすべてについてValence / Arousal双方でcriterionを満たした。これは、causal sensitivityがgeneration process全体で完全に一様ではない可能性を示す。

ただしState Induction GateがNO\_GOであり、H1--H3のreplication criteriaが一貫して成立したわけではないため、このtemporal contrast単独から普遍的なcausal mechanismを同定したとは言えない。

V3で扱うtemporal localizationは、本研究で用いたinterventionによって測定されるrelative causal leverageであり、それ以前のstageで情報が利用されていないことを意味しない。またmediated attenuationもformal causal mediationではない。


\subsection{4 Stageから得られる統合的な計算像}

以上を統合すると、Reader PredictionとSelf-Reportの関係は、
\[
\text{one shared emotion representation}
\rightarrow
\text{two output heads}
\]
のような単純な構造ではなく、本研究の結果とより整合するのは、
\[
\begin{array}{c}
\text{共通stimulusからaffect-relevant informationが形成される}
\\
\downarrow
\\
\text{ReaderとSelfの内部表現には共有可能なinformationが存在する}
\\
\downarrow
\\
\text{しかしtaskごとにcoordinate systemとcausal organizationが異なり得る}
\\
\downarrow
\\
\text{Base--Instruct間ではgeometryとtask dependenceが再編される}
\\
\downarrow
\\
\text{Self-Reportに対するlocal interventional sensitivityは}
\\
\text{model、task、layer、generation stageによって異なり得る}
\end{array}
\]
というものである。

この結果は、最終outputの類似性やlinear probeの高いdecodabilityだけから、内部mechanismの同一性を推論することの危険性を示す。


\subsection{心理的ガードレールへの含意}

心理的ガードレールという観点から見ると、本研究の結果は、単一の``emotion representation''や単一の``Self-Report layer''を抑制すれば、
他者理解能力を維持しながらself-directed affective responseだけを普遍的に制御できるという単純な設計を支持しない。

ReaderとSelfはaffect-relevant informationを部分的に共有するため、粗いinterventionはReader Predictionに必要なinformationにも影響を与える可能性がある。

一方、V1では完全なcausal interchangeabilityが支持されず、一部modelsではtask-specific specializationが観測された。

したがって、ReaderとSelfは完全に同一mechanismでもなければ、完全に独立したmechanismでもない。

このことから将来的なselective controlでは、
\[
\text{shared affect-relevant informationを保存しつつ、}
\\
\text{task-specific causal utilizationを選択的に制御する}
\]
という設計が重要になると考える。

さらにV2とV3は、そのintervention targetを全model familyへ固定的に転用することにも慎重であるべきことを示す。

実際、Base-to-Instruct recoveryはfamily-dependentであり、V3のspatial dissociationもcross-family全体で普遍的にconfirmされたわけではない（Valenceにおいて一部familyで同方向の解離が再現されたにとどまる）。

したがって実用的なguardrailを構築する場合には、各modelについて、
\[
\text{representation geometry},
\qquad
\text{task specificity},
\qquad
\text{causal leverage},
\qquad
\text{generation stage}
\]
を個別にvalidationし、Reader能力への副作用とともにintervention effectを評価する必要がある。


\subsection{解釈上の限界}

本研究におけるSelf-Reportは、standardized promptに対して生成されるmodel-generated affective reportである。

したがって、本研究の結果はLLMが人間と同様の主観的情動、affective empathy、consciousness、あるいはsentienceを持つことの証拠ではない。同様にReader Predictionも、人間心理学におけるcognitive empathyそのものではない。

本研究が対象とするのは、これら二つのoperational taskを結ぶaffect-relevant information、representation geometry、およびcausal computationである。

また、BaseとInstructは独立して得られたtrained artifactsであり、本研究ではpost-training procedureそのものをrandomized interventionしていない。したがってV2で観測された差はpost-trainingのcausal effectではなく、post-training-associated differenceとして解釈する。

さらに、4-familyおよび3-familyのcross-family analysesは、任意のLLM populationへのpopulation-level generalizationではなく、prespecified model families間におけるconsistencyの評価として位置づける。

最後に、Behavioral / V1の729-state VAD spaceと、V2 / V3の81-state VA spaceは異なるprobability spaceであるため、Stage間でabsolute expected V/Aを直接比較しない。本研究を横断して統合するのは、各Stage内部で定義したcontrast、geometry、causal effect、relative depth、およびdistributional changeである。


\subsection{結論}

以上から本研究は、Reader PredictionとSelf-Reportの関係を、behavioral covariation、internal representation、causal intervention、post-training-associated comparison、generation-stage analysisへと段階的に分解して評価した。

ReaderとSelfはaffect-relevant informationを部分的に共有する一方、そのrepresentationは直接交換可能な同一coordinate systemではなく、causal organizationにもtask-dependentな差が存在し得る。Base--Instruct間ではrepresentation geometryおよびtask-dependent causal profileに差異が観測され、そのpatternは単純なinformation erasureや一様なdepth relocationだけでは説明できなかった。

さらにV3では、decodabilityからrobustなcausal controlを直接推論できないことが示され、Gate failure後のexploratory analysisではgeneration-stage-dependentなinterventional sensitivityが観測された。ただし、robustでfamily-generalな単一のSelf-Report causal mechanismは確認されなかった。

したがって、本研究が支持する中心的な結論は、
\[
\text{Behavioral Coupling}
\not\Rightarrow
\text{Shared Representation}
...
この知見は、LLMに主観的感情が存在することを示すものではない。むしろ、他者情動推定に関わる機能を維持しながら、過度なself-directed affective responseを選択的に制御するためには、behavioral outputだけでなく、
\[
\text{representation}
\not\Rightarrow
\text{causal utilization}
\]
という区別を明示的に扱う必要があることを示す。

したがって本研究は、心理的ガードレールそのものを完成させるものではなく、将来的なselective interventionを設計・検証するためのmechanistic foundationを提供する。
