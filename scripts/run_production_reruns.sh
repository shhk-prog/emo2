...
if command -v nvidia-smi &> /dev/null; then
    ...
    echo "[Warning] nvidia-smi not found. Running on CPU/mps may be slow."
fi
...
for fam in "${FAMILIES[@]}"; do
    echo "--- V1 Phase B: Family=${fam}, task-type=reader ---"
    ${PYTHON} v1/primary/run_phase_b.py --family "${fam}" --task-type reader --force

    echo "--- V1 Phase B: Family=${fam}, task-type=self ---"
    ${PYTHON} v1/primary/run_phase_b.py --family "${fam}" --task-type self --force
done
...
${PYTHON} v3/primary/run_rq2_spatiotemporal_maps.py --family qwen --force
...
echo "================================================================="
# Discovery Model: Qwen 2.5 1.5B Instruct
${PYTHON} v3/primary/run_rq3_path_mediation.py --family qwen --force
...
for fam in "${CONF_FAMILIES[@]}"; do
    ...
    ${PYTHON} v3/primary/run_confirmatory_replication.py --family "${fam}" --force
done
...
for fam in "${FAMILIES[@]}"; do
    ...
    ${PYTHON} v2/primary/run_rq4_recovery_patching.py --family "${fam}" --force
done
