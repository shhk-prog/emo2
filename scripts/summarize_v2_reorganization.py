...
def generate_h3_lmm_table(df_h3_lmm):
    ...
    tex_lines.extend([
        ...
        r"\begin{minipage}{\linewidth}",
        r"\footnotesize",
        r"\textbf{Note:} Primary confirmatory inferenceは Alignment $\times$ Depth、Alignment $\times$ Task、Alignment $\times$ Task $\times$ Depth のprespecified interaction termsに基づく。Valenceにおいて $\text{Post-training} \times \text{Task}$ がFDR補正後も有意（$q = 0.044$）となり部分的に支持されたが、層深度の再配置（$\text{Post-training} \times \text{Depth}$等）およびArousalの全interactionはFDR補正後の基準を満たさなかった。なおBase/Instruct差はrandomized interventionではないため、post-trainingのcausal effectではなくpost-training-associated reorganizationとして解釈する。",
        r"\end{minipage}",
        r"\end{table}",
    ])
    return "\n".join(tex_lines)
