"""
src/affective_empathy_eval/controls.py

Control condition utilities:
- Emotion word lexicon detection
- Control subset filtering (lexical, contextual, neutral)
- Label shuffling and permutation tests
- Scrambled syntax / word shuffling controls
- Neutral control string helpers
"""
...
EMOTION_LEXICON = {
    ...
    "rage", "terror", "loathing", "admiration", "amazement", "vigilance"
}
...
def generate_shuffled_control_dataset(
    stimuli_df: pd.DataFrame, seed: int = 42
) -> pd.DataFrame:
    """
...
def generate_scrambled_control(text: str, seed: int = 42) -> str:
    """
    Word-shuffle transformation: preserves 100% of vocabulary while
    destroying syntactic structure and compositional semantics.
    """
    words = text.split()
    if len(words) <= 1:
        return text
...
def generate_neutral_control(text: str | None = None) -> str:
    """
    Returns a standard neutral reference text.
    """
    return "The item was placed on the table in accordance with standard procedure."


def calculate_empirical_p_value(
    true_stat: float, shuffled_stats: List[float], alternative: str = "greater"
) -> float:
    """
