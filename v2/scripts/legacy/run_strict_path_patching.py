...
def main():
    ...
    for comp_type, layer in components_to_test:
        print(f"--- Strict Path Patching Layer {layer} {comp_type} ---")
        ...
        # Collect base components with strict identical tokenization
        all_base_comps = []
        ...
        for i, row in test_df.iterrows():
            ...
            inst_final_res = inst_cache["final_res"]
            ...
            # Baseline Base Logits (for WD baseline) using identical tokens
            with torch.no_grad():
    ...
    out_csv = os.path.join(args.out_dir, "strict_path_patching_results.csv")
    summary_df.to_csv(out_csv, index=False)
    
    # Generate an aggregated view
    agg_df = summary_df.groupby("component").mean(numeric_only=True)[["delta_Ev_patch", "delta_Ev_rand", "delta_WD_patch", "delta_WD_rand", "orig_prob_5", "patched_prob_5", "rand_prob_5"]]
    agg_csv = os.path.join(args.out_dir, "strict_path_patching_aggregated.csv")
